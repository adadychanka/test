...
const Index = () => {
  ...
  useEffect(function persistTasksToLocalStorage() {
    localStorage.setItem(TASKS_STORAGE_KEY, JSON.stringify(tasks));
  }, [tasks]);

  const [editingTaskId, setEditingTaskId] = useState<string | null>(null);

  const addTask = useCallback((task: TaskFormData) => {
    const newTask: Task = {
  ...
  const editTask = useCallback((updatedTask: Partial<TaskFormData>, id: string) => {
    setTasks((prev) => {
                    ...
                    {tasks.length === 0 ? (
                      ...
                      <div className="grid gap-4">
                        {tasks.map((task) => {
                          ...
                          const dueDate = typeof task.dueDate === 'string' ? new Date(task.dueDate)?.toLocaleDateString() : task.dueDate?.toLocaleDateString();
